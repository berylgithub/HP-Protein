...
from differential_evolution import diff_evol
...
#        result = diff_evol(hp2d.func_2D_protein, bounds, hp_seq,settings, popsize=len_hp_seq*25, maxiter=300, sobol=True)
        ...
        DE_settings = {'mut':0.8, 'crossp':0.7, 'popsize':100, 'maxiter':100}
        result = cluster_DE(hp2d.F_2DHP, bounds, spiral_settings, DE_settings, hp_seq, settings, m_cluster=250, gamma=0.2, epsilon=0.5, delta=1e-1, k_cluster=20)
        
        
        print(result)
        print("coor = ", hp2d.to_2D_cartesian_from_real(result[0], settings))
#        ret_val = np.array(hp2d.return_val)
